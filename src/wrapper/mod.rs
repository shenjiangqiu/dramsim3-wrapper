...
mod tests {
    ...
    fn get_channel_id_test() {
        use super::Dramsim3Wrapper;
        let mut dramsim = Dramsim3Wrapper::new();
...
#[derive(Debug)]
pub struct Dramsim3Wrapper<'a> {
    data: &'a mut libc::c_void,
}

impl<'a> Dramsim3Wrapper<'a> {
    pub fn new() -> Self {
        unsafe {
            let dramsim3 = get_dramsim3();
...
impl<'a> Drop for Dramsim3Wrapper<'a> {
    fn drop(&mut self) {
        unsafe {
            let dramsim_ptr = self.data as *mut libc::c_void;

            delete_dramsim3(dramsim_ptr);
        }
    }
}
...
impl<'a> Dramsim3Wrapper<'a> {
    pub fn send(&mut self, addr: u64, is_write: bool) -> bool {
        unsafe {
            let dramsim_ptr = self.data as *mut libc::c_void;
            let ret = dramsim3_send(dramsim_ptr, addr, is_write as libc::boolean_t);
            match ret {
                0 => false,
                _ => true,
            }
    ...
    pub fn get(&mut self) -> u64 {
        unsafe {
            let dramsim_ptr = self.data as *mut libc::c_void;
            dramsim3_get(dramsim_ptr)
        }
    }
    pub fn tick(&mut self) {
        unsafe {
            let dramsim_ptr = self.data as *mut libc::c_void;
            dramsim3_tick(dramsim_ptr);
        }
    }
    pub fn ret_available(&mut self) -> bool {
        unsafe {
            ...
            match dramsim3_ret_available(dramsim_ptr) {
                0 => false,
                _ => true,
            }
        }
    ...
    pub fn available(&mut self, addr: u64, is_write: bool) -> bool {
        unsafe {
            ...
            match dramsim3_available(dramsim_ptr, addr, is_write as libc::boolean_t) {
                0 => false,
                _ => true,
            }
        }
    ...
    pub fn get_channel_id(&mut self, addr: u64) -> i32 {
        unsafe {
            ...
            dramsim3_get_channel_id(dramsim_ptr, addr)
        }
    }
}
